...
def generate_launch_description():
    ...
    world_path = os.path.join(get_package_share_directory('test_bot'), 'worlds', 'room.world')
    gazebo_params_file = os.path.join(get_package_share_directory('test_bot'), 'config', 'gazebo_params.yaml')
    ...
    slam_launch = ExecuteProcess(
        cmd=['ros2', 'launch', 'test_bot', 'launch_lidar_slam.py'], output='screen'
    )
